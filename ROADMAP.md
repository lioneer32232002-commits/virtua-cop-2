...
**已做（+11 測試，純函式單測）：**
- **`render/mergeStatic.js`**（純函式）：`mergeStaticMeshes(root)` traverse 收 mesh、依
  ...
  每組產一個 `Mesh`（沿用原材質/貼圖）。純粹：不動原 `root`（clone）、回傳新 Group。
...
- **`StageEnvironment.create` 接線**：`toUnlit` **之後**才 merge（順序要點：先統一 unlit 材質再分組），
  `merged.name='stage_<id>'`、原 per-mesh 幾何 `dispose()` 釋放，env.root 換成 merged group。
  fallback（無 GLB）路徑不變。`groundYAt` raycast 對 merged root 照常運作。
...
| 指標 | merge 前 | merge 後 |
|------|---------|---------|
| draw calls（stage1 開場）| 3982 | **466**（−88%）|
| 場景 stage mesh | ~9870 | **1251**（−87%）|
| geometries（drawn）| 8397 | ~458 |
| 全 stage 三角總數 | — | 65642（無遺失）|
| `groundYAt` raycast | 正常 | 正常（命中 y≈−9.8）|

- 167 game 測試全綠（156＋11 新）。preview：`environment.root.name==='stage_stage1'` 確認 merged
  group 即 env root；推進 ~4s 到 clear_point（開闊視野 ~1145/1251 mesh 入錐）draw calls 1145，
  仍遠低於 3982；全程 0 console error。
- **到不了「十位數」的原因**：1251 merged mesh＝1251 個相異（貼圖×color×layout）組——stage 用了上千張
  相異貼圖，**相異貼圖不能併同一 draw call（除非做 texture atlas）**。再往下要 atlas（更大工程，
  本刀範圍外）。88% 砍幅已是 merge-by-material 的上限收益。
