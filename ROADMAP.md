...
- [~] 效能：**已量測 stage1（2026-06-14，preview）** — drawCalls **3982**、triangles 僅 **~23K**、geometries 8397、textures 1238、場景 mesh 9870（frustum culling 砍到 3982 drawn≈60%）。結論：**draw-call bound，非三角數/far plane 問題**（23K 三角極低、far 3000 無妨、culling 正常）；瓶頸是「上千個微小獨立 mesh」（平均 ~6 三角/call）。**最大優化＝把靜態 stage 幾何依材質/貼圖 merge**（`BufferGeometryUtils.mergeGeometries`）可把 ~4000 draw call 砍到十位數 — 獨立一刀，需小心材質分組與 unlit 替換的順序，留作專門 cut。
